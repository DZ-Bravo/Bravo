// goods 컬렉션에서 title에 '남성 벤투스 윈드스토퍼 후디 다운자켓', '여성 아르테 윈드스토퍼 숏 다운 자켓'이 포함된 문서들을 top 컬렉션으로 이동
// 1. embedding_description에서 '카테고리: [상의/하의/등산화/용품]'을 '카테고리: 상의'로 변경
// 2. goods_thumbnails에서 해당하는 문서들을 top_thumbnails로 이동

use('hiking');

print("=".repeat(50));
print("goods 컬렉션에서 자켓 관련 문서를 top 컬렉션으로 이동");
print("=".repeat(50));

// 1. goods 컬렉션에서 title에 해당 키워드가 포함된 문서 찾기
print("\n1. goods 컬렉션에서 '남성 벤투스 윈드스토퍼 후디 다운자켓', '여성 아르테 윈드스토퍼 숏 다운 자켓'이 포함된 문서 검색 중...");
const keywords = ["남성 벤투스 윈드스토퍼 후디 다운자켓", "여성 아르테 윈드스토퍼 숏 다운 자켓"];
const jacketsInGoods = db.goods.find({
...
jacketsInGoods.forEach(doc => {
    ...
    // embedding_description 수정: '카테고리: [상의/하의/등산화/용품]'을 '카테고리:상의'로 변경
    let newEmbeddingDesc = embeddingDesc;
    if (embeddingDesc.match(/카테고리:\s*(상의|하의|등산화|용품)/)) {
        newEmbeddingDesc = embeddingDesc.replace(/카테고리:\s*(상의|하의|등산화|용품)/g, "카테고리:상의");
        updatedCount++;
...
const nowInTop = db.top.countDocuments(query);
