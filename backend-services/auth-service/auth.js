...
router.post('/verify-email-code', async (req, res) => {
  try {
    ...
    // Redis에서 인증번호 확인 (타임아웃 설정)
    let client = null
    try {
      client = await Promise.race([
        getRedisClient(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Redis 연결 타임아웃')), 3000))
      ])
    } catch (timeoutError) {
      console.error('Redis 연결 타임아웃:', timeoutError)
      return res.status(500).json({ error: '인증번호 확인 서비스에 일시적인 문제가 발생했습니다. 잠시 후 다시 시도해주세요.' })
    }
    
    if (!client) {
      return res.status(500).json({ error: '인증번호 확인에 실패했습니다. 잠시 후 다시 시도해주세요.' })
    }
    ...
    try {
      storedCode = await Promise.race([
        client.get(key),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Redis 조회 타임아웃')), 2000))
      ])
    } catch (queryError) {
      console.error('Redis 조회 오류:', queryError)
      return res.status(500).json({ error: '인증번호 확인 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.' })
    }
    ...
    try {
      await Promise.race([
        client.setEx(verifiedKey, 600, 'verified'), // 10분간 유지 (회원가입 완료까지 충분한 시간)
        new Promise((_, reject) => setTimeout(() => reject(new Error('Redis 저장 타임아웃')), 2000))
      ])
    } catch (saveError) {
      console.error('Redis 저장 오류:', saveError)
      // 저장 실패해도 인증은 성공했으므로 계속 진행
    }
...
router.post('/login', async (req, res) => {
  try {
    ...
    // MongoDB 연결 상태 확인
    if (mongoose.connection.readyState !== 1) {
      console.error('MongoDB 연결되지 않음, 로그인 불가')
      return res.status(503).json({ error: '데이터베이스 연결에 문제가 발생했습니다. 잠시 후 다시 시도해주세요.' })
    }
    
    // Cognito User Pool이 UsernameAttributes: email로 설정되어 있으므로
    // 먼저 MongoDB에서 사용자를 찾아 email을 가져와야 함 (타임아웃 설정)
    let user = null
    try {
      user = await Promise.race([
        User.findOne({ id }).maxTimeMS(5000),
        new Promise((_, reject) => setTimeout(() => reject(new Error('MongoDB 조회 타임아웃')), 5000))
      ])
    } catch (dbError) {
      console.error('MongoDB 조회 오류 (로그인):', dbError)
      if (dbError.message?.includes('timeout') || dbError.message?.includes('buffering') || dbError.message?.includes('ENOTFOUND')) {
        return res.status(503).json({ error: '데이터베이스 연결에 문제가 발생했습니다. 잠시 후 다시 시도해주세요.' })
      }
...
const getRedisClient = async () => {
  ...
  if (redisConnecting) {
    ...
    try {
      // 환경 변수 확인 및 로깅
      // Kubernetes 환경에서는 기본값을 Kubernetes 서비스 이름으로 설정
      const redisHost = process.env.REDIS_HOST || 'redis-0.redis.bravo-redis-ns.svc.cluster.local'
      const redisPort = parseInt(process.env.REDIS_PORT || '6379')
      
      console.log(`Redis 연결 시도: ${redisHost}:${redisPort}`)
      if (!process.env.REDIS_HOST) {
        console.warn('경고: REDIS_HOST 환경 변수가 설정되지 않았습니다. 기본값을 사용합니다.')
      }
      ...
      redisClient = createClient({
        socket: {
          ...
          reconnectStrategy: (retries) => {
            ...
            return Math.min(retries * 100, 1000) // 최대 1초 간격
          }
        }
      })
      ...
      redisClient.on('connect', () => {
        console.log(`Redis 연결 성공 (auth.js): ${redisHost}:${redisPort}`)
        redisConnecting = false
      })
      ...
      console.error('Redis 연결 실패:', error.message || error)
      redisClient = null
      redisConnecting = false
      return null
    }
...
router.post('/find-id', async (req, res) => {
  try {
    ...
    // 이메일 인증 여부 확인 (Redis에서 인증 완료 키 확인, 타임아웃 설정)
    let client = null
    try {
      client = await Promise.race([
        getRedisClient(),
      ...
      console.error('Redis 연결 타임아웃:', timeoutError)
      return res.status(503).json({ error: '인증 확인 서비스에 일시적인 문제가 발생했습니다. 잠시 후 다시 시도해주세요.' })
    }
    
    if (!client) {
      return res.status(503).json({ error: '인증 확인에 실패했습니다. 잠시 후 다시 시도해주세요.' })
    }

    const verifiedKey = `email-verification:find-id:${email}`
    let isEmailVerified = null
    try {
      isEmailVerified = await Promise.race([
        client.get(verifiedKey),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Redis 조회 타임아웃')), 2000))
      ])
    } catch (queryError) {
      console.error('Redis 조회 오류:', queryError)
      return res.status(503).json({ error: '인증 확인 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.' })
    }
    ...
    // MongoDB 연결 상태 확인
    if (mongoose.connection.readyState !== 1) {
      console.error('MongoDB 연결되지 않음, 아이디 찾기 불가')
      return res.status(503).json({ error: '데이터베이스 연결에 문제가 발생했습니다. 잠시 후 다시 시도해주세요.' })
    }

    // 사용자 찾기 (MongoDB 타임아웃 설정)
    let user = null
    try {
      user = await Promise.race([
        User.findOne({ email }).select('id name createdAt').maxTimeMS(5000).lean(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('MongoDB 조회 타임아웃')), 5000))
      ])
    } catch (dbError) {
      console.error('MongoDB 조회 오류:', dbError)
      // MongoDB 연결 실패 시 명확한 오류 메시지
      if (dbError.message?.includes('timeout') || dbError.message?.includes('buffering') || dbError.message?.includes('ENOTFOUND')) {
        return res.status(503).json({ error: '데이터베이스 연결에 문제가 발생했습니다. 잠시 후 다시 시도해주세요.' })
      }
      return res.status(500).json({ error: '아이디 찾기 중 오류가 발생했습니다.' })
    }
    ...
    try {
      await Promise.race([
        client.del(verifiedKey),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Redis 삭제 타임아웃')), 2000))
      ])
    } catch (delError) {
      console.warn('Redis 키 삭제 실패 (무시):', delError)
      // 삭제 실패해도 계속 진행
    }
...
router.post('/send-email-verification-password', async (req, res) => {
  try {
    ...
    // 아이디로 사용자 확인 (MongoDB 타임아웃 설정)
    let userById = null
    try {
      userById = await Promise.race([
        User.findOne({ id }).maxTimeMS(5000).lean(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('MongoDB 조회 타임아웃')), 5000))
      ])
    } catch (dbError) {
      console.error('MongoDB 조회 오류 (아이디):', dbError)
      if (dbError.message?.includes('timeout') || dbError.message?.includes('buffering')) {
        return res.status(503).json({ error: '데이터베이스 연결에 문제가 발생했습니다. 잠시 후 다시 시도해주세요.' })
      }
      return res.status(500).json({ error: '인증번호 전송 중 오류가 발생했습니다.' })
    }
    ...
    // 이메일로 사용자 확인 (MongoDB 타임아웃 설정)
    let userByEmail = null
    try {
      userByEmail = await Promise.race([
        User.findOne({ email }).maxTimeMS(5000).lean(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('MongoDB 조회 타임아웃')), 5000))
      ])
    } catch (dbError) {
      console.error('MongoDB 조회 오류 (이메일):', dbError)
      if (dbError.message?.includes('timeout') || dbError.message?.includes('buffering')) {
        return res.status(503).json({ error: '데이터베이스 연결에 문제가 발생했습니다. 잠시 후 다시 시도해주세요.' })
      }
      return res.status(500).json({ error: '인증번호 전송 중 오류가 발생했습니다.' })
...
router.post('/verify-email-code-password', async (req, res) => {
  try {
    ...
    // 아이디와 이메일로 사용자 확인 (MongoDB 타임아웃 설정)
    let userById = null
    try {
      userById = await Promise.race([
        User.findOne({ id }).maxTimeMS(5000).lean(),
      ...
      return res.status(500).json({ error: '인증번호 검증 중 오류가 발생했습니다.' })
    }
    ...
    try {
      await Promise.race([
        client.setEx(verifiedKey, 600, 'verified'), // 10분간 유지
        new Promise((_, reject) => setTimeout(() => reject(new Error('Redis 저장 타임아웃')), 2000))
      ])
    } catch (saveError) {
      console.warn('Redis 저장 실패 (무시):', saveError)
      // 저장 실패해도 계속 진행
    }
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    try {
      await Promise.race([
        userById.save(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('MongoDB 저장 타임아웃')), 5000))
      ])
    } catch (saveError) {
      console.error('MongoDB 저장 오류:', saveError)
