    ...
    for (const obj of objects.Contents) {
      ...
      try {
        ...
        for (const item of items) {
          ...
          // 가격 범위 검색이 있으면 최소 점수 기준을 낮춤 (0점 이상, 가격 범위에 맞으면 포함)
          // 일반 요청은 최소 3점 이상
          const minScore = hasBeginnerRequest ? 0 : (hasStickSearch ? 1 : (priceRange ? 0 : 3))
          ...
          if (score >= minScore) {
            ...
            // URL이 없거나 유효하지 않으면 제외 (항상 URL이 있어야 함)
            if (!validUrl) {
  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < recommendations.length; i++) {
        ...
        // 가격 범위가 있으면 제품명이 없어도 검색 시도
        const priceRange = parsePriceRange(userInput)
        if (!searchTitle && !hasBeginnerRequest && !priceRange) continue
