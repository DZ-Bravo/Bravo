// top 컬렉션에서 title에 '팬츠'가 포함된 문서들을 bottom 컬렉션으로 이동
...
print("=".repeat(50));
print("top 컬렉션에서 '팬츠' 문서를 bottom 컬렉션으로 이동");
print("=".repeat(50));

// 1. top 컬렉션에서 title에 '팬츠'가 포함된 문서 찾기
print("\n1. top 컬렉션에서 '팬츠'가 포함된 문서 검색 중...");
const pantsInTop = db.top.find({ title: { $regex: "팬츠", $options: "i" } }).toArray();

if (pantsInTop.length === 0) {
    print("   '팬츠'가 포함된 문서를 찾을 수 없습니다.");
    quit();
}

print(`   ${pantsInTop.length}개의 문서를 찾았습니다.`);

// 찾은 문서들의 ID와 title 출력
const pantsTitles = [];
pantsInTop.forEach(doc => {
    const docId = doc._id;
...
// 2. embedding_description 수정 및 bottom 컬렉션으로 이동
print("\n2. embedding_description 수정 및 bottom 컬렉션으로 문서 이동 중...");
let movedCount = 0;
let updatedCount = 0;
...
// 3. top_thumbnails에서 해당하는 문서들을 bottom_thumbnails로 이동
print("\n3. top_thumbnails에서 bottom_thumbnails로 문서 이동 중...");
...
// 4. 결과 확인
print("\n4. 이동 결과 확인...");
const remainingInTop = db.top.countDocuments({ title: { $regex: "팬츠", $options: "i" } });
...
const remainingInTopThumbnails = db.top_thumbnails.countDocuments({
    $or: [
        { title: { $regex: "팬츠", $options: "i" } },
        { name: { $regex: "팬츠", $options: "i" } }
    ]
});
...
print(`   - top 컬렉션에 남은 팬츠 문서: ${remainingInTop}개`);
print(`   - bottom 컬렉션에 있는 팬츠 문서: ${nowInBottom}개`);
print(`   - top_thumbnails에 남은 팬츠 썸네일: ${remainingInTopThumbnails}개`);
print(`   - bottom_thumbnails에 있는 팬츠 썸네일: ${nowInBottomThumbnails}개`);

// 5. 결과 요약
print("\n" + "=".repeat(50));
print("작업 완료!");
print("=".repeat(50));
print(`- ${movedCount}개 문서를 top → bottom으로 이동`);
print(`- ${updatedCount}개 문서의 embedding_description 수정`);
print(`- ${thumbnailsMovedCount}개 썸네일을 top_thumbnails → bottom_thumbnails로 이동`);
