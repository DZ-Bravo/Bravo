...
async function findMountainsWithoutCourses() {
  try {
    ...
    console.log('\n=== 등산 코스 정보가 없는 산 리스트 ===\n')
    console.log(`총 ${mountainsWithoutCourses.length}개\n`)
    
    mountainsWithoutCourses.forEach((m, index) => {
    ...
    return mountainsWithoutCourses
  } catch (error) {
...
findMountainsWithoutCourses().then(mountains => {
  if (mountains && mountains.length > 0) {
    console.log(`\n삭제할 산 개수: ${mountains.length}개`)
    console.log('삭제하려면 deleteMountainsWithoutCourses.js 스크립트를 실행하세요.')
  }
  process.exit(0)
})
