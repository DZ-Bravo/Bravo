...
function MountainDetail({ name, code, height, location, description, center, zoom, origin, image, cctvUrl }) {
  ...
  const proj4ReadyRef = useRef(false)
  const proj4LoadingRef = useRef(null)
  ...
  useEffect(() => {
    ...
    const loadKakaoMap = () => {
      ...
      if (window.kakao && window.kakao.maps) {
        ...
        script.src = `https://dapi.kakao.com/v2/maps/sdk.js?appkey=${apiKey}&autoload=false&_=${cacheBust}`
        script.async = true
        script.onload = () => {
          if (window.kakao && window.kakao.maps) {
            window.kakao.maps.load(() => {
  ...
  // 지도 중심 좌표를 안전하게 반환 (숫자 아니면 기본값 사용)
  const getSafeCenter = () => {
    let lat
    let lon
    ...
    if (Array.isArray(center)) {
      ...
      lat = Number(center.lat ?? center[0])
      lon = Number(center.lon ?? center.lng ?? center[1])
    }

    if (!Number.isFinite(lat) || !Number.isFinite(lon)) {
      return { lat: 36.5, lon: 127.8, isFallback: true }
    }
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      try {
        ...
        const { lat, lon, isFallback } = getSafeCenter()
        if (isFallback) {
          console.warn('[MountainDetail.jsx] center가 유효하지 않아 기본값으로 대체합니다.', { center, lat, lon })
        }
  ...
  // 코스 데이터가 로드되고 지도가 준비되면 자동으로 표시 - 비활성화 (NaN 에러 방지)
  useEffect(() => {
    // 이 로직을 완전히 비활성화하고, 대신 loadCourseData에서 첫 번째 코스만 표시
    return
    
    /* 비활성화된 코드
    if (!code || courses.length === 0 || !mapInstanceRef.current || !window.kakao?.maps) {
      return
    }
    
    // 지도가 준비되고 코스 데이터가 있으면 지도에 표시
    if (mapInstanceRef.current && courses.length > 0 && courseLayerRef.current.length === 0) {
      console.log('코스 데이터와 지도가 준비됨, 지도에 코스 표시 시작')
      // loadCourseData를 다시 호출하지 않고 직접 표시
      const geoJsonData = {
      ...
      if (geoJsonData.features && geoJsonData.features.length > 0) {
        ...
        geoJsonData.features.forEach((feature, idx) => {
          ...
          try {
            ...
            console.error(`코스 ${idx} 좌표 처리 중 오류:`, err)
            path = []
          }
          
          if (path.length >= 2) {
            // Polyline 생성 직전 최종 검증: path의 모든 LatLng가 유효한지 확인
            const validPath = path.filter(latLng => {
              try {
            ...
            if (validPath.length >= 2) {
              try {
                ...
                console.error(`코스 ${idx} Polyline 생성 실패:`, polylineError)
              }
            }
          }
        })
        
        if (hasValidBounds && minLat !== Infinity && Number.isFinite(minLat) && Number.isFinite(maxLat) && 
            Number.isFinite(minLng) && Number.isFinite(maxLng)) {
          const latDiff = maxLat - minLat
          const lngDiff = maxLng - minLng
          const padding = Math.max(latDiff * 0.1, lngDiff * 0.1, 0.005)
    ...
    */
  }, [courses, code, mapInstanceRef.current, kakaoLoaded])
  ...
  // 모든 코스를 지도에 그리는 함수 (토글 X, 항상 표시)
  const renderAllCourses = async (courseList = courses) => {
        ...
        geoJsonData.features.forEach((feature) => {
          ...
          try {
            if (feature.geometry.type === 'LineString') {
              path = feature.geometry.coordinates
                ...
                .map(coord => new window.kakao.maps.LatLng(coord[1], coord[0]))
            } else if (feature.geometry.type === 'MultiLineString') {
              path = feature.geometry.coordinates.flat()
                .filter(coord => {
                  const lng = Number(coord[0])
            ...
            console.error(`코스 ${courseName} 좌표 처리 중 오류:`, err)
            path = []
          }
          ...
          if (validPath.length >= 2) {
            try {
              ...
              console.error(`코스 ${courseName} Polyline 생성 실패:`, polylineError)
              return // 이 코스만 스킵
            }
            
            if (!polyline) {
              console.error(`코스 ${courseName} Polyline이 생성되지 않았습니다`)
              return
            }
            ...
            // 경로의 각 점을 bounds에 추가 (수동 계산) - 먼저 처리
            ...
            const midPosition = validPath[midIndex]
            
            // midPosition이 유효한지 확인
            if (midPosition && Number.isFinite(midPosition.getLat()) && Number.isFinite(midPosition.getLng())) {
              ...
              console.warn('midPosition이 무효합니다, 코스 라벨 표시 건너뜀:', courseName)
            }
          }
        })
  ...
  const showAllCourses = async () => {
    ...
    // 단일 코스가 선택되어 있으면 (selectedCourseIndex !== null) 전체 코스로 전환
    // 전체 코스가 이미 표시되어 있고 (selectedCourseIndex === null && coursesVisible) 코스 레이어가 있으면 숨기기
    if (selectedCourseIndex === null && coursesVisible && courseLayerRef.current.length > 0) {
      // 기존 이벤트 리스너 제거
      eventListenersRef.current.forEach(listener => {
        if (listener && listener.remove) {
          window.kakao.maps.event.removeListener(listener.target, listener.type, listener.handler)
    ...
    // 단일 코스가 선택되어 있거나, 코스가 표시되지 않았으면 전체 코스를 표시
    // 기존 레이어 제거 (단일 코스 또는 숨겨진 상태에서 전체 코스로 전환)
    courseLayerRef.current.forEach(polyline => {
      if (polyline && polyline.setMap) {
        polyline.setMap(null)
      }
  ...
  const displayCourseOnMap = async (course, index) => {
    if (!mapInstanceRef.current || !window.kakao || !window.kakao.maps) {
      ...
      setTimeout(() => {
        if (mapInstanceRef.current && course) {
          ...
          // 재시도 실패 시 기본 지도라도 보이도록 설정
          console.warn('지도 초기화 재시도 실패, 기본 지도를 표시합니다.')
          if (mapInstanceRef.current && center) {
            try {
              const { lat, lon } = getSafeCenter()
              if (Number.isFinite(lat) && Number.isFinite(lon)) {
                const mapCenter = new window.kakao.maps.LatLng(lat, lon)
                mapInstanceRef.current.setCenter(mapCenter)
                mapInstanceRef.current.setLevel(zoom || 13)
              }
            } catch (e) {
              console.error('기본 지도 center 설정 실패:', e)
            }
          }
        }
      }, 500)
    ...
    try {
      ...
      // geometry 유효성 검사 (ArcGIS와 GeoJSON 모두 고려)
      if (!courseToDisplay?.geometry) {
        console.warn('코스 geometry가 없어 표시를 건너뜁니다.', courseToDisplay)
        setCoursesVisible(false)
        return
      }
      
      // ArcGIS 형식이면 paths가 있어야 하고, GeoJSON 형식이면 coordinates가 있어야 함
      if (isArcGISFormat && !courseToDisplay.geometry.paths) {
        console.warn('ArcGIS 형식 코스에 paths가 없어 표시를 건너뜁니다.', courseToDisplay)
        setCoursesVisible(false)
        return
      }
      
      if (!isArcGISFormat && !courseToDisplay.geometry.coordinates) {
        console.warn('GeoJSON 형식 코스에 coordinates가 없어 표시를 건너뜁니다.', courseToDisplay)
        setCoursesVisible(false)
        return
      }
      ...
      if (geoJsonData.features && geoJsonData.features.length > 0) {
        ...
        if (!feature.geometry || !feature.geometry.coordinates) {
          console.warn('코스 geometry 없음', { courseName, geometry: feature.geometry })
          setCoursesVisible(false)
          return
        }
        
        // 좌표 추출 (유효 좌표만 사용, 중첩 배열 flatten)
        const extractPairs = (coords) => {
          const pairs = []
          const walk = (arr) => {
            if (!Array.isArray(arr)) return
            if (arr.length === 2 && arr.every(v => v !== null && v !== undefined)) {
              const lng = Number(arr[0])
              const lat = Number(arr[1])
              // 좌표 범위는 한반도/동아시아 넓은 구간으로 완화
              if (Number.isFinite(lat) && Number.isFinite(lng) && lat >= 15 && lat <= 55 && lng >= 110 && lng <= 150) {
                pairs.push([lng, lat])
              }
              return
            }
        ...
        const buildLatLng = (coordsArr = []) => {
          const valid = []
          const pairs = extractPairs(coordsArr)
          pairs.forEach(([lng, lat]) => {
            try {
              // LatLng 생성 전 최종 검증
              if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
                console.warn('LatLng 생성 전 무효 좌표 감지:', { lat, lng, courseName })
                return
              }
              ...
              // 생성 후에도 검증
              if (Number.isFinite(latLng.getLat()) && Number.isFinite(latLng.getLng())) {
                bounds.extend(latLng)
                valid.push(latLng)
              } else {
                console.warn('LatLng 생성 후 무효 좌표 감지:', { lat, lng, courseName })
              }
            } catch (err) {
              console.warn('LatLng 생성 중 오류:', { lat, lng, courseName, error: err.message })
            }
          })
          return valid
        }
        ...
        const isValidLatLng = (latLng) => {
          if (!latLng || !latLng.getLat || !latLng.getLng) return false
          const lat = latLng.getLat()
          const lng = latLng.getLng()
          return Number.isFinite(lat) && Number.isFinite(lng) && lat >= 15 && lat <= 55 && lng >= 110 && lng <= 150
        }

        let path = []
        if (feature.geometry.type === 'LineString') {
          path = buildLatLng(feature.geometry.coordinates)
        } else if (feature.geometry.type === 'MultiLineString') {
          path = buildLatLng(feature.geometry.coordinates)
        } else {
          console.warn('지원하지 않는 geometry type, 표시 건너뜀:', feature.geometry.type)
          setCoursesVisible(false)
          return
        }
        
        // 유효 좌표만 최종 필터링
        path = path.filter(isValidLatLng)

        // 최종 경로 검증 (유효한 좌표만 남기고, 2개 미만이면 스킵)
        path = path.filter(isValidLatLng)
        if (path.length < 2) {
          console.warn('코스 경로에 유효한 좌표가 부족하여 표시 건너뜀.', {
            courseName,
        ...
        // Polyline 생성 직전 최종 검증 (NaN 완전 차단)
        const finalValidPath = path.filter(latLng => {
          try {
            const lat = latLng.getLat()
            const lng = latLng.getLng()
            return Number.isFinite(lat) && Number.isFinite(lng)
        ...
        let polyline = null
        if (finalValidPath.length >= 2) {
          // Polyline 생성 (예외 방지용 try/catch)
          try {
            console.log(`[코스 표시] Polyline 생성 시작: ${courseName}, 경로 포인트 수: ${finalValidPath.length}`)
            polyline = new window.kakao.maps.Polyline({
              path: finalValidPath,
              strokeWeight: 5,
              strokeColor: difficultyColor,
            ...
            polyline.setMap(mapInstanceRef.current)
            courseLayerRef.current.push(polyline)
            console.log(`[코스 표시] Polyline 생성 및 지도 추가 완료: ${courseName}`)
          } catch (err) {
            console.error('Polyline 생성 실패, 코스 표시 스킵:', courseName, err)
            setCoursesVisible(false)
            return
          }
        } else {
          console.warn('최종 검증 후 유효한 좌표가 부족합니다:', courseName, finalValidPath.length)
          // 코스 표시 실패해도 기본 지도는 보이도록 함
          if (mapInstanceRef.current && center) {
            try {
              const { lat, lon } = getSafeCenter()
              if (Number.isFinite(lat) && Number.isFinite(lon)) {
        ...
        if (!polyline) {
          console.warn('Polyline이 생성되지 않았습니다')
          // 코스 표시 실패해도 기본 지도는 보이도록 함
          if (mapInstanceRef.current && center) {
            try {
              const { lat, lon } = getSafeCenter()
        ...
        if (finalValidPath.length > 0) {
          ...
          const startPoint = finalValidPath[0]
          const endPoint = finalValidPath[finalValidPath.length - 1]
          ...
          for (let i = 0; i < finalValidPath.length - 1; i++) {
            const p1 = finalValidPath[i]
            const p2 = finalValidPath[i + 1]
          ...
          samplePoints.push({ lat: endPoint.getLat(), lon: endPoint.getLng(), index: finalValidPath.length - 1 })
          ...
          try {
            if (finalValidPath.length > 0) {
              // bounds가 비어있으면 finalValidPath에서 직접 계산
              let minLat = Infinity, maxLat = -Infinity, minLng = Infinity, maxLng = -Infinity
              
              finalValidPath.forEach(latLng => {
                const lat = latLng.getLat()
                const lng = latLng.getLng()
                // NaN 체크 추가
                if (Number.isFinite(lat) && Number.isFinite(lng)) {
              ...
              if (minLat !== Infinity && maxLat !== -Infinity && minLng !== Infinity && maxLng !== -Infinity &&
                ...
                // padding도 검증
                if (!Number.isFinite(paddingLat) || !Number.isFinite(paddingLng)) {
                  console.warn('padding이 무효합니다')
                  return
                }
          ...
          console.warn('코스 경로에 유효한 좌표가 없습니다.', { courseName, geometry: feature.geometry })
          setCoursesVisible(false)
          return
        }
      }
      ...
      if (mapInstanceRef.current && center) {
        try {
          ...
          if (Number.isFinite(lat) && Number.isFinite(lon)) {
            ...
            mapInstanceRef.current.setCenter(mapCenter)
            mapInstanceRef.current.setLevel(zoom || 13)
            console.log('코스 표시 실패 후 기본 지도 center 설정 완료')
          }
        } catch (e) {
          console.error('기본 지도 center 설정 실패:', e)
        }
  ...
  // proj4js 로더 (ArcGIS 좌표 변환용)
  const ensureProj4 = async () => {
    if (proj4ReadyRef.current && window.proj4) return true
    if (window.proj4) {
      proj4ReadyRef.current = true
      return true
    }

    if (!proj4LoadingRef.current) {
      proj4LoadingRef.current = new Promise((resolve) => {
        ...
        // 1차: cdnjs, 실패 시 2차: jsdelivr
        loadScript('https://cdnjs.cloudflare.com/ajax/libs/proj4js/2.9.2/proj4.js', () => {
          console.warn('proj4js cdnjs 로드 실패, jsdelivr로 재시도')
          loadScript('https://cdn.jsdelivr.net/npm/proj4@2.9.2/dist/proj4.js', () => {
            console.error('proj4js 로드 실패 (모든 CDN)')
            resolve(false)
          })
        })
      })
    ...
    return proj4LoadingRef.current
  }
  ...
  const loadCourseData = async (mountainCode, map) => {
    ...
    try {
      ...
      const projReady = await ensureProj4()
      if (!projReady) {
        console.warn('proj4js가 로드되지 않아 코스 좌표 변환이 실패할 수 있습니다.')
      }
      // 백엔드 API에서 데이터 가져오기
      const apiUrl = API_URL
      ...
      if (response.status === 404) {
        console.warn('코스 API 404: 해당 산의 코스 데이터 없음', mountainCode)
        setCourses([])
        setSelectedCourseIndex(null)
        setCoursesVisible(false)
      ...
      if (data.courses && data.courses.length > 0) {
        // 시간/거리 문자열에서 숫자 추출 헬퍼
        const extractNumber = (value) => {
          if (value === undefined || value === null) return NaN
          if (typeof value === 'number') return Number.isFinite(value) ? value : NaN
          if (typeof value === 'string') {
            // "5분", "0.18km", "5", "0.18" 같은 형식에서 숫자만 추출
        ...
        let coursesData = data.courses.filter(c => {
          ...
          // 디버깅: 필터링 전 값 확인 (모든 코스에 대해)
          console.log(`[필터링 체크] ${props.name || '이름 없음'}: 원본 거리=${distRaw}, 원본 시간=${durRaw}, 추출 거리=${dist}, 추출 시간=${dur}`)
          
          // 10분 미만 AND 0.5km 미만인 경우만 제외
          if (Number.isFinite(dist) && Number.isFinite(dur) && dist < 0.5 && dur < 10) {
            console.log(`[필터링] 코스 제외: ${props.name || '이름 없음'}, 거리=${dist}km, 시간=${dur}분`)
            return false
          }
          
          // 둘 중 하나라도 유효하지 않으면 통과 (필터링 안 함)
          if (!Number.isFinite(dist) || !Number.isFinite(dur)) {
            console.log(`[필터링] 통과 (값 없음): ${props.name || '이름 없음'}, 거리=${dist}, 시간=${dur}`)
            return true
          }
        ...
        // 유효한 값이 있는 코스만 카운트 (값 없음 코스 제외)
        const validCoursesCount = coursesData.filter(c => {
          const props = c.properties || {}
          const distRaw = props.distance ?? props.PMNTN_LT ?? props.courseLength
          const durRaw = props.duration ?? props.upTime ?? props.PMNTN_UPPL
          const dist = extractNumber(distRaw)
          ...
          return Number.isFinite(dist) && Number.isFinite(dur)
        }).length
        
        console.log(`[필터링] 원본 코스: ${data.courses.length}개, 필터링 후: ${coursesData.length}개, 유효한 코스: ${validCoursesCount}개`)
        
        // 유효한 값이 있는 코스가 0개이면 가장 긴 코스 1개를 강제로 남김
        if (validCoursesCount === 0 && data.courses.length > 0) {
          // 모든 코스가 걸러진 경우: 가장 긴 시간(동일 시 거리) 코스 1개를 강제로 남김
          coursesData = [...data.courses].sort((a, b) => {
            const pa = a.properties || {}
            const pb = b.properties || {}
            
            // 시간 추출 (upTime + downTime 고려)
            let durA = extractNumber(pa.duration ?? pa.upTime ?? pa.PMNTN_UPPL) || 0
            let durB = extractNumber(pb.duration ?? pb.upTime ?? pb.PMNTN_UPPL) || 0
            
            if (!durA && (pa.upTime || pa.downTime)) {
              const upTime = extractNumber(pa.upTime) || 0
              const downTime = extractNumber(pa.downTime) || 0
              durA = upTime + downTime
            }
            
            if (!durB && (pb.upTime || pb.downTime)) {
              const upTime = extractNumber(pb.upTime) || 0
              const downTime = extractNumber(pb.downTime) || 0
              durB = upTime + downTime
            }
            
            if (durA !== durB) return durB - durA
            
            const distA = extractNumber(pa.distance ?? pa.PMNTN_LT) || 0
            const distB = extractNumber(pb.distance ?? pb.PMNTN_LT) || 0
            return distB - distA
          }).slice(0, 1)
          console.log(`[필터링] 유효한 코스가 없어 가장 긴 코스 1개 유지:`, coursesData[0]?.properties?.name || '이름 없음')
        }
        setCourses(coursesData)
        
        // URL 파라미터에서 코스 이름 확인 (있으면 선택만 설정)
        ...
        // URL에 지정된 코스가 없으면 첫 번째 코스를 기본 선택
        if (initialCourseIndex === null) {
        ...
        // URL에 코스 이름이 있으면 해당 코스만 표시, 없으면 모든 코스 표시
        const courseNameFromUrlForDisplay = searchParams.get('course')
        if (courseNameFromUrlForDisplay && initialCourseIndex !== null) {
          // URL에 코스 이름이 있으면 해당 코스만 표시
          setTimeout(() => {
            if (mapInstanceRef.current && window.kakao && window.kakao.maps) {
              const targetCourse = coursesData[initialCourseIndex]
              if (targetCourse) {
                displayCourseOnMap(targetCourse, initialCourseIndex)
              }
            }
          }, 500)
        } else {
          // URL에 코스 이름이 없으면 모든 코스 표시
          setTimeout(() => {
            if (mapInstanceRef.current && window.kakao && window.kakao.maps) {
        ...
        // URL 파라미터 처리 완료, loadCourseData 함수 종료
        setCoursesLoading(false)
      } else {
        setCourses([])
        setSelectedCourseIndex(null)
        ...
        if (mapInstanceRef.current && center) {
          try {
            ...
            if (Number.isFinite(lat) && Number.isFinite(lon)) {
              ...
              mapInstanceRef.current.setCenter(mapCenter)
              mapInstanceRef.current.setLevel(zoom || 13)
              console.log('코스 로드 실패 후 기본 지도 center 설정 완료')
            }
          } catch (e) {
            console.error('기본 지도 center 설정 실패:', e)
          }
  ...
  const getLodgingLatLng = (lodging) => {
    ...
    return { lat: toNumberOrNull(lat), lon: toNumberOrNull(lon) }
  }
  ...
  const loadLodgingData = async (mountainCode) => {
    try {
      ...
      console.log(`[숙소] 받은 데이터: ${list.length}개 (mountain_lodging 컬렉션에서 이미 매칭된 데이터)`)

      // 백엔드에서 이미 필터링된 데이터이므로 그대로 사용
      // place_id/id/이름+주소 기준으로 중복 제거만 수행
      const deduped = []
      const seen = new Set()
      list.forEach(item => {
        const key =
          item.place_id ||
          item.id ||
          `${item.name || item.title || item.lodging?.name || ''}_${item.address || item.road_address || ''}`
        if (!seen.has(key)) {
          seen.add(key)
          deduped.push(item)
        }
      ...
      console.log(`[숙소] 최종 데이터: ${deduped.length}개 (중복 제거 후)`)
      const finalList = deduped

      setLodgings(finalList)
      return finalList
    } catch (error) {
  ...
  const toggleLodgingMarkers = async () => {
    ...
    // 이미 마커가 표시된 경우 -> 마커만 제거하고 종료 (코스 상태 유지)
    ...
    try {
      ...
      const filtered = list.filter(item => {
        const codeFromItem =
          ...
          item.mountain?.code ||
          item.mountain?.id
        return codeFromItem ? String(codeFromItem) === String(mountainCode) : true
      })

      // place_id/id/이름+주소로 중복 제거
      const deduped = []
      const seen = new Set()
      filtered.forEach(item => {
        const key =
          item.place_id ||
          item.id ||
          `${item.name || item.title || ''}_${item.address || item.road_address || ''}`
        if (!seen.has(key)) {
          seen.add(key)
          deduped.push(item)
        }
      ...
      // 현재 산 중심 반경 내(예: 25km)만 남김
      const { lat: centerLat, lon: centerLon } = getSafeCenter()
      const NEAR_KM = 25
      const nearby = deduped.filter(item => {
        const { lat, lon } = getRestaurantLatLng(item)
        if (!Number.isFinite(lat) || !Number.isFinite(lon)) return false
        const distKm = calculateDistance(centerLat, centerLon, lat, lon) / 1000
        return Number.isFinite(distKm) && distKm <= NEAR_KM
      })

      const finalList = nearby.length > 0 ? nearby : deduped
  ...
  useEffect(() => {
    const fetchFavoriteStatus = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/api/auth/mountains/${code}/favorite`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        })
          ...
          <section className="section course-main-section">
            ...
            <div className="course-layout">
                          ...
                          <div 
                            ...
                            onClick={async () => {
                              ...
                              if (!mapInstanceRef.current) {
                                ...
                                const checkMap = setInterval(() => {
                                  if (mapInstanceRef.current || retries >= 20) {
                                    ...
                                    if (mapInstanceRef.current) {
                                      ...
                                      setTimeout(() => {
                                        console.log(`[코스 클릭] 지도 준비 완료, 코스 표시 시작`)
                                ...
                                // 약간의 지연 후 코스 표시 (지도가 완전히 준비된 후)
                                console.log(`[코스 클릭] 지도 준비됨, 코스 표시 시작`)
                                setTimeout(() => {
                                  displayCourseOnMap(courseToDisplay, actualIndex)
                                }, 100)
                              }
              ...
              <div className="course-detail-panel">
                ...
                {/* 지도 컨테이너 */}
                <div className="map-container" style={{ position: 'relative', width: '100%', height: '600px', minHeight: '500px' }}>
                  {/* 2D/3D 전환 버튼 */}
                  <button
